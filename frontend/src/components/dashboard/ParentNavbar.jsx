...
const ParentNavbar = ({
  ...
  return (
    <header className="sticky top-0 z-30 flex h-20 w-full items-center justify-between border-b border-slate-200 bg-white/95 px-4 backdrop-blur-md lg:px-8 shadow-2xs">
        ...
        {students.length > 1 ? (
          <div className="flex items-center gap-2 rounded-2xl border border-[#BFDBFE] bg-[#EBF5FF] px-3 py-1.5 shadow-2xs">
            ...
            <select
              ...
              className="bg-transparent text-xs font-black text-[#0C4A86] focus:outline-none cursor-pointer"
            >
              {students.map((st) => {
                const id = st._id || st.userId?._id || st.userId;
                const name = `${st.userId?.firstName || st.name || 'Child'} ${st.userId?.lastName || ''}`.trim();
                const cls = st.grade || st.class?.grade ? ` (Grade ${st.grade || st.class?.grade})` : '';
                return (
          ...
          {isProfileMenuOpen && (
            <div className="absolute right-0 mt-2 w-56 rounded-3xl border border-slate-200 bg-white p-2 shadow-2xl space-y-1">
              <div className="border-b border-slate-100 p-3">
                ...
                <p className="text-[11px] text-slate-500 font-semibold">{user?.email || 'parent@school.com'}</p>
              </div>
              <button
                onClick={() => {
                ...
                className="flex w-full items-center gap-2.5 rounded-xl px-3 py-2 text-xs font-bold text-slate-700 hover:bg-slate-50"
              >
              ...
              <button
                ...
                className="flex w-full items-center gap-2.5 rounded-xl px-3 py-2 text-xs font-extrabold text-rose-600 hover:bg-rose-50"
              >
