...
import { CheckCircle2, XCircle, Calendar, AlertCircle, Filter, Clock, ChevronLeft, ChevronRight, ArrowLeft } from 'lucide-react';
...
const sampleAttendanceRecords = [
  ...
  { date: '2026-08-29', status: 'Weekend' },
  { date: '2026-08-30', status: 'Weekend' },
...
const ParentAttendance = ({ selectedStudentId }) => {
  ...
  useEffect(() => {
    const loadAttendance = async () => {
      ...
      try {
        setLoading(true);
        const response = await attendanceService.getByStudent(selectedStudentId);
    ...
    <div className="space-y-6">
      ...
      <div className="rounded-3xl bg-gradient-to-r from-[#0C4A86] to-[#0096DA] p-6 text-white shadow-md flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <h1 className="text-2xl font-bold">Child's Attendance Summary</h1>
          <p className="text-sky-100 text-sm">Calendar date-wise attendance view, working days & percentage calculation.</p>
        </div>
      ...
      {/* View Toggle Bar & Month Selector */}
      <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm flex flex-col sm:flex-row items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          <span className="text-xs font-bold text-slate-500 uppercase flex items-center gap-1.5 whitespace-nowrap">
            <Filter className="h-4 w-4 text-[#0C4A86]" /> View Mode:
          </span>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm space-y-1">
          <p className="text-xs text-slate-400 font-bold uppercase tracking-wider">Attendance Percentage</p>
