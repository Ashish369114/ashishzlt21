  ...
  return (
    <div style={{ padding: '24px', background: '#FAF6F0', minHeight: '100vh' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '24px' }}>
        <div>
          <h2 style={{ fontSize: '20px', fontWeight: '800', color: '#0C4A86', margin: 0, display: 'flex', alignItems: 'center', gap: '10px' }}>
            <FileText size={22} color="#0C4A86" /> Minutes of Meeting (MOM) - Staff & PTM
          </h2>
        ...
        <div style={{ display: 'flex', gap: '10px' }}>
          ...
          <button onClick={handleOpenAddModal} style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '8px 16px', background: '#0C4A86', border: 'none', borderRadius: '10px', fontSize: '13px', fontWeight: '600', color: '#ffffff', cursor: 'pointer' }}>
      ...
      <div style={{ background: '#ffffff', border: '1px solid #BFDBFE', borderRadius: '14px', padding: '16px', marginBottom: '20px', display: 'flex', flexWrap: 'wrap', gap: '12px', alignItems: 'center' }}>
        ...
        <select value={meetingTypeFilter} onChange={(e) => { setMeetingTypeFilter(e.target.value); setPage(1); }} style={{ padding: '9px 12px', borderRadius: '8px', border: '1px solid #BFDBFE', fontSize: '13px', outline: 'none', color: '#0C4A86' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(340px, 1fr))', gap: '16px', marginBottom: '20px' }}>
        {loading ? (
          ...
          <div key={mom.id || mom._id} style={{ background: '#ffffff', border: '1px solid #BFDBFE', borderRadius: '14px', padding: '18px', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', boxShadow: '0 2px 8px rgba(0,0,0,0.03)' }}>
            <div>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '10px' }}>
                <span style={{
                  padding: '4px 10px', borderRadius: '20px', fontSize: '11px', fontWeight: '700', textTransform: 'uppercase',
                  background: mom.meetingType === 'staff_meeting' ? '#FAF6F0' : '#FEF3C7',
                  color: mom.meetingType === 'staff_meeting' ? '#0C4A86' : '#B45309'
                }}>
                  {mom.meetingType === 'staff_meeting' ? 'Staff Meeting' : 'PTM Meeting'}
                </span>
                <div style={{ display: 'flex', gap: '6px' }}>
                  <button onClick={() => handleOpenEditModal(mom)} style={{ border: 'none', background: '#FAF6F0', padding: '6px', borderRadius: '6px', cursor: 'pointer', color: '#0C4A86' }} title="Edit">
                    <Edit size={14} />
                  </button>
              ...
              <h3 style={{ fontSize: '16px', fontWeight: '700', color: '#0C4A86', margin: '0 0 10px' }}>{mom.title}</h3>
              ...
              {mom.agenda && (
                <div style={{ marginBottom: '10px' }}>
                  <strong style={{ fontSize: '12px', color: '#0C4A86', display: 'block', marginBottom: '2px' }}>Agenda:</strong>
                  <p style={{ fontSize: '12px', color: '#6B5B54', margin: 0, lineHeight: '1.4' }}>{mom.agenda}</p>
                </div>
              )}

              {mom.keyDecisions && (
                <div style={{ marginBottom: '10px', background: '#FAF6F0', padding: '8px 10px', borderRadius: '8px', borderLeft: '3px solid #0C4A86' }}>
                  <strong style={{ fontSize: '11px', color: '#0C4A86', display: 'block', marginBottom: '2px' }}>Key Decisions:</strong>
                  <p style={{ fontSize: '12px', color: '#6B5B54', margin: 0, lineHeight: '1.4' }}>{mom.keyDecisions}</p>
                </div>
              )}
            </div>

            <div style={{ borderTop: '1px solid #FAF6F0', paddingTop: '10px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', fontSize: '11px', color: '#8C7F79' }}>
      ...
      {/* Pagination */}
      <div style={{ background: '#ffffff', border: '1px solid #BFDBFE', borderRadius: '12px', padding: '12px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
      ...
      {showModal && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(50, 32, 41, 0.5)', backdropFilter: 'blur(4px)', zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
          <div style={{ background: '#ffffff', borderRadius: '16px', width: '100%', maxWidth: '600px', maxHeight: '90vh', overflowY: 'auto', padding: '24px', boxShadow: '0 20px 40px rgba(0,0,0,0.15)', border: '1px solid #BFDBFE' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '18px', borderBottom: '1px solid #FAF6F0', paddingBottom: '12px' }}>
              <h3 style={{ fontSize: '18px', fontWeight: '800', margin: 0, color: '#0C4A86' }}>{editingId ? 'Edit Meeting MOM' : 'Create New Meeting MOM'}</h3>
            ...
            <form onSubmit={handleSave} style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
              <div>
                <label style={{ display: 'block', fontSize: '12px', fontWeight: '700', color: '#0C4A86', marginBottom: '4px' }}>Meeting Title *</label>
                <input type="text" required value={formData.title} onChange={e => setFormData({ ...formData, title: e.target.value })} style={{ width: '100%', padding: '9px 12px', borderRadius: '8px', border: '1px solid #BFDBFE', fontSize: '13px' }} placeholder="e.g. Monthly Staff Academic Planning Meeting" />
              </div>

              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                <div>
                  <label style={{ display: 'block', fontSize: '12px', fontWeight: '700', color: '#0C4A86', marginBottom: '4px' }}>Meeting Type *</label>
                  <select value={formData.meetingType} onChange={e => setFormData({ ...formData, meetingType: e.target.value })} style={{ width: '100%', padding: '9px 12px', borderRadius: '8px', border: '1px solid #BFDBFE', fontSize: '13px' }}>
                    <option value="staff_meeting">Staff Meeting</option>
                    <option value="ptm_meeting">PTM Meeting</option>
                  </select>
                </div>
                <div>
                  <label style={{ display: 'block', fontSize: '12px', fontWeight: '700', color: '#0C4A86', marginBottom: '4px' }}>Date *</label>
                  <input type="date" required value={formData.meetingDate} onChange={e => setFormData({ ...formData, meetingDate: e.target.value })} style={{ width: '100%', padding: '9px 12px', borderRadius: '8px', border: '1px solid #BFDBFE', fontSize: '13px' }} />
                </div>
              </div>

              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                <div>
                  <label style={{ display: 'block', fontSize: '12px', fontWeight: '700', color: '#0C4A86', marginBottom: '4px' }}>Time</label>
                  <input type="text" value={formData.time} onChange={e => setFormData({ ...formData, time: e.target.value })} style={{ width: '100%', padding: '9px 12px', borderRadius: '8px', border: '1px solid #BFDBFE', fontSize: '13px' }} placeholder="10:00 AM" />
                </div>
                <div>
                  <label style={{ display: 'block', fontSize: '12px', fontWeight: '700', color: '#0C4A86', marginBottom: '4px' }}>Venue</label>
                  <input type="text" value={formData.venue} onChange={e => setFormData({ ...formData, venue: e.target.value })} style={{ width: '100%', padding: '9px 12px', borderRadius: '8px', border: '1px solid #BFDBFE', fontSize: '13px' }} placeholder="Conference Room A" />
                </div>
              </div>

              <div>
                <label style={{ display: 'block', fontSize: '12px', fontWeight: '700', color: '#0C4A86', marginBottom: '4px' }}>Agenda</label>
                <textarea rows="2" value={formData.agenda} onChange={e => setFormData({ ...formData, agenda: e.target.value })} style={{ width: '100%', padding: '9px 12px', borderRadius: '8px', border: '1px solid #BFDBFE', fontSize: '13px' }} placeholder="Main topics discussed..."></textarea>
              </div>

              <div>
                <label style={{ display: 'block', fontSize: '12px', fontWeight: '700', color: '#0C4A86', marginBottom: '4px' }}>Key Decisions</label>
                <textarea rows="2" value={formData.keyDecisions} onChange={e => setFormData({ ...formData, keyDecisions: e.target.value })} style={{ width: '100%', padding: '9px 12px', borderRadius: '8px', border: '1px solid #BFDBFE', fontSize: '13px' }} placeholder="Decisions agreed upon..."></textarea>
              </div>

              <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '10px', marginTop: '10px' }}>
                <button type="button" onClick={() => setShowModal(false)} style={{ padding: '9px 18px', background: '#ffffff', border: '1px solid #BFDBFE', borderRadius: '8px', fontSize: '13px', fontWeight: '600', cursor: 'pointer', color: '#0C4A86' }}>Cancel</button>
                <button type="submit" style={{ padding: '9px 20px', background: '#0C4A86', color: '#fff', border: 'none', borderRadius: '8px', fontSize: '13px', fontWeight: '700', cursor: 'pointer' }}>Save Record</button>
              </div>
            </form>
          </div>
        </div>
