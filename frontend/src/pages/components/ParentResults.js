...
import { Award, TrendingUp, Eye, ChevronDown, Filter, Search, BookOpen, CheckCircle2, X, UserCheck, ArrowLeft } from 'lucide-react';
...
const ParentResults = ({ selectedStudentId, student }) => {
  const navigate = useNavigate();
  const [selectedExamType, setSelectedExamType] = useState(() => {
    return localStorage.getItem('parent_selected_exam') || 'unit_test_1';
  });
  ...
  useEffect(() => {
    const handleStorageSync = () => {
      const stored = localStorage.getItem('parent_selected_exam');
  ...
  const handleExamChange = (e) => {
    ...
    localStorage.setItem('parent_selected_exam', newExam);
  };
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Synchronized 7-Stage Exam Selection Controls */}
      <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm flex flex-col sm:flex-row items-center justify-between gap-4">
        <div className="flex items-center gap-3 w-full sm:w-auto">
          <label className="text-xs font-bold text-slate-500 uppercase flex items-center gap-1.5 whitespace-nowrap">
            <Filter className="h-4 w-4 text-[#0C4A86]" /> Select Examination Term:
          </label>
          ...
          <React.Fragment key={type.id}>
            ...
            <button
              onClick={() => {
                ...
                localStorage.setItem('parent_selected_exam', type.id);
              }}
